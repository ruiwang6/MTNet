...
def get_dataloader(args, normalizer='std', tod=False, dow=False, weather=False, single=True):
    ...
    scaler1 = StandardScaler(mean=data_train[..., 0].mean(), std=data_train[..., 0].std())
    scaler2 = StandardScaler(mean=data_train[..., 1].mean(), std=data_train[..., 1].std())

    data_train[..., 0] = scaler1.transform(data_train[..., 0])
    data_val[..., 0] = scaler1.transform(data_val[..., 0])
    data_test[..., 0] = scaler1.transform(data_test[..., 0])

    data_train[..., 1] = scaler2.transform(data_train[..., 1])
    data_val[..., 1] = scaler2.transform(data_val[..., 1])
    data_test[..., 1] = scaler2.transform(data_test[..., 1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_steps', default=12, type=int)
    parser.add_argument('--horizon', default=12, type=int)
    parser.add_argument('--batch_size', default=64, type=int)
    args = parser.parse_args()
    train_dataloader, val_dataloader, test_dataloader, scaler1, scaler2 = get_dataloader(args, normalizer='std',
                                                                                         tod=False,
                                                                                         dow=False, weather=False,
                                                                                         single=True)
